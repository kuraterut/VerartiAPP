...
//        Map<Long, List<Appointment>> dayInfo = Connection.getMastersSheduleByDate(token, date);
	...
	public static BorderPane loadWindow(LocalDate date){
        ...
        addNewClientBtn.setOnAction(event -> showAddNewClientDialog(addNewClientBtn, date));
...
//                e.printStackTrace(); // Добавьте логирование для отладки
    ...
    public static void showCreateMasterDialog(){
        Stage dialog = new Stage();
        dialog.initModality(Modality.APPLICATION_MODAL);
        ...
        addMasterBtn.setOnAction(event -> {
            ...
            Response addingClientResponse = Connection.addNewUser(token, user);
    ...
    public static void showAddNewClientDialog(Node node, LocalDate date) {
        Stage dialog = new Stage();
        dialog.initModality(Modality.APPLICATION_MODAL);
